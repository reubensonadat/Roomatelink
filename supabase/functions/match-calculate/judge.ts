// ═══════════════════════════════════════════════════════════════════
// ROOMMATE LINK — MATCHING ALGORITHM JUDGE (PURE LOGIC)
// ═══════════════════════════════════════════════════════════════════
// This file contains ONLY pure math and logic functions.
// No database fetching. No HTTP handling. No framework dependencies.
// ═══════════════════════════════════════════════════════════════════
...
import { CATEGORIES, MAX_WEIGHTED_SCORE, VISIBILITY_THRESHOLD } from './types.ts';
...
function detectCrossCategoryPatterns(userA: AnswerVector, userB: AnswerVector): PatternFlag[] {
  ...
  if (aIsImposer && bIsImposer) {
    ...
    // Single imposer — check if other respects boundaries
    const otherRespectsQ28 = aIsImposer ? userB.q28 === 1 : userA.q28 === 1;
    if (!otherRespectsQ28) {
      flags.push({
        id: 'SINGLE_IMPOSER',
        description: 'One user imposes; other is not fully boundary-respecting',
        penalty: 8,
  ...
  if (aLostParty && bLostParty) {
    ...
    // Single lost-party user — check if other provides structure
    ...
    if (otherIsStructured) {
      ...
      flags.push({
        id: 'LOST_WITH_ANCHOR',
        description: 'One user is still finding themselves; other provides structure — potentially stabilising',
        penalty: 0,
  ...
  if (aParticular && bParticular) {
    ...
    // One particular, one not — friction if other is easygoing about belongings
    ...
    if (otherIsEasygoing) {
      flags.push({
        ...
        description: 'One user is very particular about belongings; other treats shared space casually',
        penalty: 10,
  ...
  // One user's partner visits frequently; other feels invaded.
...
function analyseConsistency(answers: AnswerVector): {
  ...
  // This flag is informational — it doesn't change score,
  // but outlier answers get 1.5× weight in base calculation.
  ...
  // If Q40 self-assessment contradicts dominant pattern.
...
/**
 * Calculate full compatibility result between two users.
 *
 * This is main entry point for matching algorithm.
 * It runs all three layers and produces final match percentage
 * along with a complete breakdown for UI.
 *
...
export function calculateMatch(userA: AnswerVector, userB: AnswerVector): MatchResult {
  ...
  // Use worse modifier (more suspicious profile drags down confidence)
...
/**
 * Convert a user's letter-based answers (from localStorage/UI) to numeric AnswerVector
 */
 ...
 * This is what the Edge Function calls after a user finishes their questionnaire.
 *
