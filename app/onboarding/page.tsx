...
import { Sun, Moon, Lock, ChevronRight, Check, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row transition-colors duration-300">
      ...
      <div className="w-full md:w-1/2 p-6 md:p-12 lg:px-24 lg:py-20 flex flex-col min-h-[50vh] md:min-h-screen">
            ...
            {currentSlide < 3 ? (
              ...
              >
                ...
                <div className="flex flex-col gap-4 max-w-sm">
                  <div className="relative flex bg-muted p-2 rounded-3xl border border-border/40">
                    <button
                      onClick={() => setTheme("light")}
                      className={`flex items-center justify-center gap-3 flex-1 py-4.5 rounded-2xl transition-all duration-300 relative z-10 font-black text-[15px] ${!isDark ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
                    >
                    ...
                    <div
                      className={`absolute top-2 bottom-2 w-[calc(50%-8px)] rounded-2xl transition-all duration-500 bg-card shadow-xl border border-white/5 ${!isDark ? 'left-2' : 'left-[calc(50%+4px)]'}`}
                    />
                  </div>
                </div>
              </motion.div>
        ...
        <div className="pt-12">
          <button
            onClick={handleNext}
            className="w-full md:w-auto md:min-w-[240px] py-5 px-10 rounded-2xl bg-foreground text-background font-black text-[18px] shadow-2xl active:scale-[0.98] hover:scale-[1.02] transition-all flex items-center justify-center gap-3 group"
          >
