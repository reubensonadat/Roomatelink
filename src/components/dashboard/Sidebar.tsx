...
export function Sidebar() {
  ...
  // Profile completion logic: checks if core fields are filled
  const isComplete = !!(profile?.course && profile?.level && profile?.phone_number);
    ...
    <aside className="hidden md:flex flex-col w-64 h-screen fixed left-0 top-0 border-r border-border bg-background/50 backdrop-blur-xl z-40 p-6">
      ...
      <div className="mt-auto pt-6 flex items-center gap-3 px-3 cursor-pointer hover:opacity-75 transition-opacity">
        <div className={cn(
          ...
          isComplete ? "bg-primary/10 border-primary/20" : "bg-muted border-border"
        )}>
          {isComplete ? (
            profile?.avatar_url ? (
        ...
        <div className="flex flex-col overflow-hidden">
          ...
          <span className={cn(
            "text-xs font-medium",
            isComplete ? "text-primary" : "text-foreground/50"
          )}>
